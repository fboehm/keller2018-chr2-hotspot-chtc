...
pp2 <- m2_out %>% 
  left_join(res) %>%
  filter(!(local_gene_symbol == "Hnf4a")) %>%
  ggplot() + aes(x = lrt, y = lod_diff_proportion, colour = nonlocal_gene_symbol) + geom_point(size = 1 / 4) + broman::karl_theme() + theme(legend.position="none") + ylab("LOD difference proportion") + xlab("Pleiotropy vs. separate QTL test statistic") + ylim(0, 1) + facet_wrap(~ local_gene_symbol, ncol = 2)
ggsave("lod-diff-prop-v-lrt-12-panels.jpg")
ggsave("lod-diff-prop-v-lrt-12-panels.eps")
ggsave("lod-diff-prop-v-lrt-12-panels.svg")
