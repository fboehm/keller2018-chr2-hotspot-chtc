...
cbind(dimnames(dri21)[[3]], indices)
```
```{r}
...
pp <- myt3 %>%
  ggplot() + aes(x = lrt, y = lod_diff_proportion * (lod_diff_proportion > 0), colour = local_gene_symbol != "Hnf4a") + geom_point(size = 1 / 10) + broman::karl_theme() +  theme(legend.position="none") + ylab("LOD difference proportion") + xlab("Pleiotropy vs. separate QTL test statistic") + ylim(0, 1) + xlim(0, 75)
ggplotly(pp, tooltip = c("colour", "x", "y")) 
...
pp <- myt3 %>%
  ...
  ggplot() + aes(x = lrt, y = lod_diff_proportion * (lod_diff_proportion > 0), colour = local_gene_symbol != "Hnf4a") + geom_point(size = 1 / 4) + broman::karl_theme() + theme(legend.position="none") + ylab("LOD difference proportion") + xlab("Pleiotropy vs. separate QTL test statistic") + ylim(0, 1) + xlim(0, 75)
ggplotly(pp, tooltip = c("colour", "x", "y")) 
...
for (i in 1:12){
  foo %>%
    ...
    ggplot() + geom_point(aes(x = lrt, y = lod_diff_proportion * (lod_diff_proportion > 0), colour = local_gene_symbol != "Hnf4a" ), size = 1/10) + broman::karl_theme(strip.placement = "inside") + theme(legend.position="none") + ylab("LOD difference proportion") + xlab("Pleiotropy vs. separate QTL test statistic") + ylim(0, 1) + xlim(0, xmax)
...
# separate plots for each local gene
foo %>%
  ggplot() + geom_point(aes(x = lrt, y = lod_diff_proportion * (lod_diff_proportion > 0), colour = local_gene_symbol != "Hnf4a"), size = 1/10) + broman::karl_theme() + facet_wrap(~ local_gene_symbol, nrow = 6, ncol = 2) + theme(legend.position="none") + #theme(
...
# separate plots for each local gene
foo %>%
  ggplot() + geom_point(aes(x = lrt, y = lod_diff_proportion * (lod_diff_proportion > 0)), size = 1/10, colour = "powderblue") + broman::karl_theme() + facet_wrap(~ local_gene_symbol, nrow = 3, ncol = 4) + theme(legend.position="none") + 
  theme(
    strip.background = element_blank(),
    strip.text.x = element_blank()
) + 
  ...
  xlab("Pleiotropy vs. separate QTL test statistic") + 
  ylim(0, 1) + xlim(0, NA) +
  geom_text(data = foo, aes(x/2 + 5, y, label = local_gene_symbol), inherit.aes=FALSE)
...
ggsave(paste0(fn, "-wide.svg"), height = 6, width = 9, units = "in")
...
## Does qtl2::scan1 - without random effects - give the LOD scores in the Keller RData file?

```{r}
hh <- hot[, colnames(hot) == "ENSMUSG00000003746", drop = FALSE]
scan1(genoprobs = genoprobs, pheno = hh, kinship = NULL, addcovar = addcovar, reml = FALSE, cores = 6
        ) -> s1_hotspot_fixed_only

```

```{r}
find_peaks(s1_hotspot_fixed_only, map = map)
```
