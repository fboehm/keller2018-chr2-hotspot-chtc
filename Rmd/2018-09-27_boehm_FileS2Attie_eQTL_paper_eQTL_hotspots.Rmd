...
Load the Data Dryad RData file.
...
I think that these are the peaks in the object `dataset.islet.rnaseq$lod.peaks`.

```{r}
lod_summary <- dataset.islet.rnaseq$lod.peaks %>%
  rename(qtl_lod = lod, qtl_pos = pos, qtl_chr = chrom) %>%
  left_join(dataset.islet.rnaseq$annots, by = c("annot.id" = "gene_id")) %>%
  as_tibble() %>%
  rename(gene_chr = chr, gene_start = start, gene_end = end) %>%
...
for(i in 1:length(hotspot.genes)) {
  tmp = data.frame(ensembl = hotspot.genes[[i]]$annot.id, t(dataset.islet.rnaseq$expr[,hotspot.genes[[i]]$annot.id]))
...
covar <- dataset.islet.rnaseq$covar
...
expr.df <- dataset.islet.rnaseq$expr %>%
  as_tibble() %>%
  mutate(mouse = rownames(dataset.islet.rnaseq$expr))
...
names(mediators) = hotspot.peaks$chr
  ...
  for(i in 1:nrow(hotspot.peaks)) {
    ...
    df = left_join(hs.pc1, gp, by = "mouse") %>%
         ...
         left_join(y = dataset.clinical.phenotypes$covar %>% 
                     as_tibble() %>% 
                     mutate(mouse = rownames(dataset.clinical.phenotypes$covar)), 
                   by = "mouse") %>% 
         #select(mouse, sex, DOwave, pc1, B:H) %>%
        select(mouse, sexM, DOwave2, DOwave3, DOwave4, pc1, B:H) %>%
        left_join(y = dataset.islet.rnaseq$expr %>%
                    as_tibble() %>%
                    mutate(mouse = rownames(dataset.islet.rnaseq$expr)), 
                  by = "mouse") %>%
         #gather(ensembl, expr, -mouse, -sex, -pc1, -DOwave, -(B:H)) %>%
         gather(annot.id, dataset.islet.rnaseq$expr %>%
                    as_tibble() %>%
                    mutate(mouse = rownames(dataset.islet.rnaseq$expr)), 
                -mouse, -sexM, -pc1, 
                -DOwave2, - DOwave3, - DOwave4, -(B:H)) %>%
      #select(mouse, pc1, sex, DOwave, expr, B:H, ensembl) %>%
       select(mouse, pc1, sexM, 
              DOwave2, DOwave3, DOwave4, 
              dataset.islet.rnaseq$expr %>%
                    as_tibble() %>%
                    mutate(mouse = rownames(dataset.islet.rnaseq$expr)), B:H, annot.id) %>%
        #group_by(ensembl) %>%
      group_by(annot.id) %>%   
      nest()
    df = df %>% 
