...
Load the Attie data file from Data Dryad.
...
Define cis traits as those with gene start position within 4 Mb of QTL peak position and on same chromosome.

```{r fig.width=10,fig.height=10,warning=FALSE,message=FALSE}
lod_summary <- dataset.islet.rnaseq$lod.peaks %>%
  left_join(dataset.islet.rnaseq$annots, by = c("annot.id" = "gene_id"))
lod_summary = mutate(lod_summary, cis = (chrom == chr) & (abs(start - pos) < 4))
```
...
tmp[[i]] =  lod_summary %>%
             filter(lod >= 7.18 & cis == FALSE) %>%
             arrange(chrom, pos) %>%
             group_by(chrom) %>%
             mutate(win = cut(pos, breaks = breaks[,i])) %>%
             group_by(chrom, win) %>% 
             summarize(cnt = n()) %>%
...
pdf(paste0("trans_eqtl_density.pdf"), width = 10, height = 8)
print(out.plot)
dev.off()
out.plot
```
...
pdf(paste0("cis_eqtl_density.pdf"), width = 10, height = 8)
print(out.plot)
dev.off()
out.plot
```
...
```{r select_eqtl_genes}
hotspot.genes = as.list(hotspots$chrom)
names(hotspot.genes) = hotspots$chrom
for(i in 1:nrow(hotspots)) {
  hotspot.genes[[i]] = lod_summary %>% 
                         filter(lod >= 7.18) %>%
                         filter(chrom == hotspots$chrom[i] & 
                           pos >= hotspots$proximal[i] & 
                           pos <= hotspots$distal[i] &
                           (chr != hotspots$chrom[i] |
                           (chr == hotspots$chrom[i] &
                            start > hotspots$distal[i] + 1 &
                            end < hotspots$proximal[i] - 1)))
  write_csv(hotspot.genes[[i]], path = paste0("chr", names(hotspot.genes)[i], "_hotspot_genes.csv"))
}
```
...
cis.hotspot.genes = as.list(cis.hotspots$chrom)
names(cis.hotspot.genes) = cis.hotspots$chrom
for(i in 1:nrow(cis.hotspots)) {
  cis.hotspot.genes[[i]] = lod_summary %>% 
                             select(annot.id, marker.id, chr, pos, lod) %>%
                             filter(lod >= 7.18) %>%
                             filter(chr == cis.hotspots$chr[i] & 
                                    pos >= cis.hotspots$proximal[i] & 
                                    pos <= cis.hotspots$distal[i])
  write_csv(cis.hotspot.genes[[i]], path = paste0( "chr", names(cis.hotspot.genes)[i], "_cis_hotspot_genes.csv"))
}
```
...
expr.mrna <- dataset.islet.rnaseq$expr
for(i in 1:length(hotspot.genes)) {
  tmp = data.frame(ensembl = hotspot.genes[[i]]$annot.id, t(expr.mrna[,hotspot.genes[[i]]$annot.id]))
  hotspot.genes[[i]] = left_join(hotspot.genes[[i]], tmp, by = c("annot.id"="ensembl"))
  write_csv(hotspot.genes[[i]], path = paste0("chr", names(hotspot.genes)[i], "_hotspot_genes.csv"))
}
```
...
for(i in 1:length(hotspot.genes)) {
  ...
  side.colors = cut(hotspot.genes[[i]]$lod, breaks = 100)
  ...
  pdf(paste0("hotspot_gene_cor_chr", chr, ".pdf"), width = 10, height = 10)
